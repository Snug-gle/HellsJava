...
//				}
		...
		if (userRole != null) {
			String role = userRole.role().toString();
			if ("USER_PRETRAINER".equals(role)) {
		...
		if (userRole != null) {
			...
			if ("ALL".equals(role)) {
				if (!(authUser.getMemberStatus() == 1 || authUser.getMemberStatus() == 2
						|| authUser.getMemberStatus() == 3 || authUser.getMemberStatus() == 9)) {
					response.sendRedirect(request.getContextPath());
					return false;
				}
			}
		...
		if (userRole != null) {
			String role = userRole.role().toString();
			if ("PRETRAINER_TRAINER".equals(role)) {
				if (!(authUser.getMemberStatus() == 2 || authUser.getMemberStatus() == 3)) {
					response.sendRedirect(request.getContextPath());
					return false;
				}
			}
		...
		if (userRole != null) {
			String role = userRole.role().toString();
			if ("USER_PRETRAINER_ADMIN".equals(role)) {
				if (!(authUser.getMemberStatus() == 1 || authUser.getMemberStatus() == 2
						|| authUser.getMemberStatus() == 9)) {
					response.sendRedirect(request.getContextPath());
					return false;
				}
			}
		...
		// 사용예시 EX) 컨트롤러의 메소드 위 또는 클래스위에 @Auth or @Auth(role=Role.ADMIN) or
		// @Auth(role=Role.TRAINER) 등으로 사용
