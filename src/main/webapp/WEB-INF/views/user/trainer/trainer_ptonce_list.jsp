...
<script type="text/javascript">
	...
	if (${pager.startPage} > ${pager.blockSize}) {
		html += "<a href='<c:url value="/ptonce/trainer/list/status"/>?pageNum=1&ptOnceStatus="+$("#category").val()+"'>[처음]</a>";
		html += "<a href='<c:url value="/ptonce/trainer/list/status"/>?pageNum=${pager.prevPage}&ptOnceStatus="+$("#category").val()+"'>[<]</a>";
	} else {
		html += "[처음][<]";
	}
	//페이지 번호 설정
	for (var i = ${pager.startPage}; i <= ${pager.endPage}; i++) {
		if (${pager.pageNum} != i) {
			html += "<a href='<c:url value="/ptonce/trainer/list/status"/>?pageNum="+i+"&ptOnceStatus="+$("#category").val()+"'> [" + i + "] </a>";
		} else {
			html += "[" + i + "]";
		}
	}
	...
	if (${pager.endPage} != ${pager.totalPage}) {
		html += "<a href='<c:url value="/ptonce/trainer/list/status"/>?pageNum=${pager.nextPage}&ptOnceStatus="+$("#category").val()+"'>[>]</a>";
		html += "<a href='<c:url value="/ptonce/trainer/list/status"/>?pageNum=${pager.totalPage}&ptOnceStatus="+$("#category").val()+"'>[마지막]</a>";
	} else {
		html += "[>][마지막]";
	}
	//페이징 처리 결과 출력
